...
def tool_commit_experience(summary: str, win_rate: float, compiler_status: str) -> str:
    """
    Commit an experiment result to the Evolution History Vector Database AND save as a Markdown file.
    ...
    doc_id = f"exp_{timestamp}_{uuid.uuid4().hex[:4]}"
    ...
    # 3. 儲存至實體 Markdown 檔案 (供人類查閱)
    from config import AKAGI_BASE_PATH
    log_dir = os.path.join(AKAGI_BASE_PATH, "docs", "lesson_learned")
    os.makedirs(log_dir, exist_ok=True)
    
    file_path = os.path.join(log_dir, f"{doc_id}.md")
    content = f"""# Experiment Post-Mortem: {doc_id}
Date: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
Status: {compiler_status.upper()}
Win Rate: {win_rate}%
...
---
*Generated by Akagi R&D Agent Memory System*
"""
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(content)
    
    return f"Successfully saved experiment to VectorDB and local file: docs/lesson_learned/{doc_id}.md"
